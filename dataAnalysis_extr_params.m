...
% calib params were estimated and hence don't need to be changed
calibration_params = [f_x, f_y, p_x, p_y];
unknown_params = [x_cam, y_cam, z_cam, phi_cam, theta_cam, psi_cam];
all_data = [cf_data', pel_data];

projected_points_exp2 = ProjectToCameraPlane(unknown_params, calibration_params, all_data);
...
% MAIN ERROR
mean_euclidean_distance_error = MeanEuclideanDistanceError(unknown_params, calibration_params, all_data);
fprintf('Mean Euclidean distance error: %.2f\n', mean_euclidean_distance_error);
% OTHER ERRORS 
mean_absolute_error = MeanAbsoluteError(unknown_params, calibration_params, all_data);
fprintf('Mean absolute error: %.2f\n', mean_absolute_error);
mean_squared_error = MeanSquaredError(unknown_params, calibration_params, all_data);
fprintf('Mean absolute error: %.2f\n', mean_squared_error);
...
% ALPHA TRIMMED MED
alpha_med = AlphaTrimmedMeanEuclideanDistanceError(unknown_params, calibration_params, all_data, alpha);
fprintf('Alpha-trimmed mean Euclidean distance error: %.2f\n', alpha_med);
% OTHER ONES
alpha_mae = AlphaTrimmedMeanAbsoluteError(unknown_params, calibration_params, all_data, alpha);
fprintf('Alpha-trimmed mean absolute error: %.2f\n', alpha_mae);
alpha_mse = AlphaTrimmedMeanSquaredError(unknown_params, calibration_params, all_data, alpha);
fprintf('Alpha-trimmed mean squared error: %.2f\n', alpha_mse);


% OPTIMISATION
optimal_MED_params = fminsearch(@(params) AlphaTrimmedMeanEuclideanDistanceError(params, calibration_params, all_data, alpha), unknown_params);
optimal_MAE_params = fminsearch(@(params) AlphaTrimmedMeanAbsoluteError(params, calibration_params, all_data, alpha), unknown_params);
optimal_MSE_params = fminsearch(@(params) AlphaTrimmedMeanSquaredError(params, calibration_params, all_data, alpha), unknown_params);

fprintf('Initial parameters: \n');
fprintf('%.2f ', unknown_params);
fprintf('\n');
...
min_mean_euclidean_distance_error = AlphaTrimmedMeanEuclideanDistanceError(optimal_MED_params, calibration_params, all_data, alpha);
fprintf('Optimal mean Euclidean distance error: %.2f\n', min_mean_euclidean_distance_error);
% others
min_mean_absolute_error = AlphaTrimmedMeanAbsoluteError(optimal_MAE_params, calibration_params, all_data, alpha);
fprintf('Optimal mean absolute error: %.2f\n', min_mean_absolute_error);
min_mean_squared_error = AlphaTrimmedMeanSquaredError(optimal_MSE_params, calibration_params, all_data, alpha);
fprintf('Optimal mean squared error: %.2f\n', min_mean_squared_error);

med_projected_points = ProjectToCameraPlane(optimal_MED_params, calibration_params, all_data);
...
title('Projected Sensor Coordinates (min. MAE) vs Ground Truth Coordinates');
xlabel('X-coordinate (pixels)');
ylabel('Y-coordinate (pixels)');
grid on;
set(gca, 'YDir', 'reverse');
...
exp3_projected_points = ProjectToCameraPlane(optimal_MED_params, calibration_params, final_sensor_readings);
...
projection_back_to_3d = ProjectImageTo3D(unknown_params, calibration_params, ground_truth_image_coords);
MEDopt_projection_back_to_3d = ProjectImageTo3D(optimal_MED_params, calibration_params, ground_truth_image_coords);
...
% exp3_min_reproj_errors = sum(abs(MEDopt_projection_back_to_3d - measured_ground_truth));
% exp3_reproj_errors = sum(abs(projection_back_to_3d - measured_ground_truth));
...
title('Plot of GT Reprojections (6 Params.) vs GT Measurements');
...
% 3D TO 2D
function projected_points = ProjectToCameraPlane(params, calibration_params, data)
    f_x = calibration_params(1); % 11/04 CHANGE PARAMS TO INCL. UNKNOWN PARAMS
    f_y = calibration_params(2);
...
function projection_back_to_3d = ProjectImageTo3D(params, calibration_params, projected_points)
    f_x = calibration_params(1); % CHANGE TO INCLUDE UNKNOWN PARAMS
    f_y = calibration_params(2);
    p_x = calibration_params(3);
    p_y = calibration_params(4);
    x_cam = params(1);
...
function med = MeanEuclideanDistanceError(params, calibration_params, data)
...
function alpha_med = AlphaTrimmedMeanEuclideanDistanceError(params, calibration_params, data, alpha)
    f_x = calibration_params(1);
    f_y = calibration_params(2); %xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    p_x = calibration_params(3);
    p_y = calibration_params(4);
...
%%%%%%%%% MEAN ABSOLUTE ERROR!!!
function mae = MeanAbsoluteError(params, calibration_params, data)
    f_x = calibration_params(1);
    f_y = calibration_params(2); %xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    p_x = calibration_params(3);
    p_y = calibration_params(4);
...
%%%%%%%%%%%%%%%%%% MEAN SQUARED ERROR
function mse = MeanSquaredError(params, calibration_params, data)
    f_x = calibration_params(1);
    f_y = calibration_params(2); %xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    p_x = calibration_params(3);
    p_y = calibration_params(4);
...
function alpha_mse = AlphaTrimmedMeanSquaredError(params, calibration_params, data, alpha)
    f_x = calibration_params(1);
    f_y = calibration_params(2); %xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    p_x = calibration_params(3);
    p_y = calibration_params(4);
